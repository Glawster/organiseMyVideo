...
def testConfirmedCameraImportReportsByteProgress(tmp_path: Path):
    source, media = cameraSourceCreate(tmp_path)
    progress = []
    planner = CameraImportPlanner(
        goproDestination=tmp_path / "archive" / "GoPro",
    ...
    importer = CameraImporter(
        ...
        manifestDirectory=tmp_path / "state" / "camera-imports",
        dryRun=False,
        progressCallback=lambda copied, total, name: progress.append(
            (copied, total, name)
        ),
    )

    result = importer.importMedia(source)

    totalBytes = media.stat().st_size
    assert result.copied == 1
    assert progress[0] == (0, totalBytes, "")
    assert progress[-1] == (totalBytes, totalBytes, media.name)
    assert all(total == totalBytes for _, total, _ in progress)
