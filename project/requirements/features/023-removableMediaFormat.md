...
- Treat the requested label as intent only until the post-format block-device label
  has been read back and verified. A confirmed format must fail rather than report
  success if the actual label is not `Card18`.
- After formatting, explicitly apply the label with the filesystem-specific label
  utility where required, wait for udev/device metadata to settle, and verify the
  actual label from the block device before relying on the remount path.
...
- A confirmed summary must report the verified post-format label and the actual
  remounted path, not merely the requested label or the pre-format mount path.
...
1. Given card 18 whose latest non-empty snapshot is completely archived and has no
   ...
   removable device and reports `Card18` as the requested new volume label without
   mutation.
2. Given the same card and `-y`, the filesystem is reformatted, explicitly labelled
   `Card18`, the block-device label is read back and verified as `Card18`, the medium
   is remounted, the durable card identity is recreated, and a fresh empty inventory
...
10. Given formatting succeeds but the actual post-format block-device label differs
    from the requested `CardNN`, the operation reports an error and must not print a
    confirmed-success summary or persist the fresh empty inventory snapshot.
11. Given a confirmed successful format, the summary displays the verified label and
    actual remounted path rather than the pre-format values from the planning stage.

## Change history

- 2026-09-17: require post-format volume-label read-back verification and actual
  remount-path reporting after a format of card 7 remained mounted as `Card18` while
  the command incorrectly reported `Label: Card7`.
