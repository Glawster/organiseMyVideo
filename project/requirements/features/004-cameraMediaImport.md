...
- Carry `cardId` through import planning/results and persist it in confirmed
  import evidence.
- Show the resolved numbered card in the normal camera-import summary.
- Add `camera import --list` for all confirmed import-manifest history and
  `camera import --card ID --list` for durable card-specific history without
  requiring the card to be mounted.
...
25. Given confirmed camera-import manifests exist, when
    `camera import --list` runs, then all readable prior confirmed imports are
    summarized in reverse chronological order, including durable card identity
    where known, outcome counts, date, and diagnostic source path.
26. Given `camera import --card 1 --list`, when import history is read, then only
    manifests whose durable `cardId` is 1 are shown and no mounted source path
    is required to identify that card.
27. Given normal camera import planning or execution, when a card identity is
...
- History tests covering card reuse, multiple inventory snapshots for one card,
  import reconciliation to the correct `snapshotId`, all-card import listing,
  and `--card ID --list` filtering.
...
- Tests: `tests/test_cameraPlan.py`, `tests/test_cameraImport.py`,
  `tests/test_cameraImportHistory.py`, `tests/test_cli.py`.
...
- 2026-09-12: added durable import/card/snapshot identity requirements so card
  ...
  coincidence.
- 2026-09-13: implemented card-aware import summaries, durable `importId` and
  `cardId` manifest fields, `camera import --list`, and
